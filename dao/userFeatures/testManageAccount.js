...
async function createAccountWithRole(accountData, roleName) {
  try {
    await connectDB();
    ...
    try {
      ...
      // Get the transportation value from the account data
      let transportationValue = accountData.Transportation || '';

      // Insert a new role into Roles table and get the generated RoleID
      let insertRoleQuery = `
        INSERT INTO Roles (RoleName, Transportation, BonusPoints, NumberOfOrdersDelivered, Rank)
        OUTPUT INSERTED.RoleID
        VALUES (@RoleName, @Transportation, 0, 0, '');
      `;
      let roleResult = await transaction.request()
        .input('RoleName', sql.VarChar(50), roleName)
        .input('Transportation', sql.VarChar(100), transportationValue)
        .query(insertRoleQuery);
