...
const {
  UpdateAccount,
  accountStatus,
} = require("../../dao/userFeatures/UpdateAccount");
...
router.get("/history-order", async (req, res) => {
  ...
  if (!email) {
    return res
      .status(400)
      .json({ status: false, message: "Please input CustomerID to Query" });
  }
  try {
    ...
    if (historyOrder.recordset.length > 0) {
      ...
      return res
        ...
        .json({
          status: true,
          message:
            "History Order is Empty. Buy something luxurious to fill it up",
        });
    }
  } catch (error) {
...
router.put("/change-locate", async (req, res) => {
  ...
  try {
    ...
    const chkStatus = await db
      ...
      ); // o for OrderTable, od for OrderDetail
    ...
    if (orderStatus === "Pending") {
      ...
      return res
        ...
        .json({
          status: false,
          message: "Order is processing and you cannot change location",
        });
    }
  } catch (error) {
    console.error("Error:", error);
...
// // Route to delete schedule appointment by ID
// router.delete('/schedule-appointments/:id', async (req, res) => {
//   const scheduleId = req.params.id;
//   try {
//     const success = await deleteScheduleAppointment(scheduleId);
//     if (success) {
//       res.status(200).json({ message: 'Schedule appointment deleted successfully' });
//     } else {
//       res.status(404).json({ message: 'Schedule appointment not found' });
//     }
//   } catch (error) {
